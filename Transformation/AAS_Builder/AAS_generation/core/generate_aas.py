...
# AAS conformance; no standalone object-store validator ships with the project.
# This no-op keeps the legacy generate_all() path functional.
class _NoOpValidationResult:
    ...
    @staticmethod
    def summary() -> str:
        return "Validation handled by SHACL (Validation.Validator.run_shacl)"
...
class AASGenerator:
    ...
    def _ensure_ontology_guidance(self) -> None:
        ...
        hints = [s for s in suggestions if s["action"] == "hint"]
        if hints:
            print(f"SHACL guidance ({len(hints)} hint(s)):")
            for s in hints[:10]:
                print(f"  - {s['description']}")
