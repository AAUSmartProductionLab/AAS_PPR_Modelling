...
import {
  ...
  AID_INTERFACE, AID_INTERACTION_METADATA,
  WOT_ACTION_AFFORDANCE, WOT_PROPERTY_AFFORDANCE,
  CAPABILITY_SET, CAPABILITY_CONTAINER,
  SEMANTIC_ID_BASE,
...
// ── Lookup tables (derived from the submodel registry) ────────────────────────

const SUBMODEL_SEMANTIC_ID = Object.fromEntries(
  SUBMODEL_KEYS.map((k) => [k, SUBMODELS[k].semanticId]),
) as Record<SubmodelKey, string>;

// idShort each builder uses as path segment.
const SUBMODEL_IDSHORT = Object.fromEntries(
  SUBMODEL_KEYS.map((k) => [k, SUBMODELS[k].idShort]),
) as Record<SubmodelKey, string>;
