...
// One catalog section per AAS type. Submodels only drag onto a matching shell.
const CATALOG_TYPES: { type: AASType; title: string }[] = [
  { type: 'Resource', title: 'Resource Submodels' },
  { type: 'Product', title: 'Product Submodels' },
];
...
export function CatalogPanel() {
  ...
  const onDragStartSubmodel = (e: React.DragEvent, key: SubmodelKey, aasType: AASType) => {
    e.dataTransfer.setData('application/submodel-key', key);
    // Carried so the drop target can reject cross-type drops early.
    e.dataTransfer.setData('application/submodel-aastype', aasType);
  ...
  return (
    <aside className="mb-catalog" style={{ width: panelWidth }}>
      ...
      <div className="mb-catalog__inner">
        ...
        {/* ── Submodel sections, one per AAS type (fills remaining height) ── */}
        <div className="mb-catalog__section mb-catalog__section--grow">
          {CATALOG_TYPES.map(({ type, title }) => {
            const keys = submodelKeysForType(type);
            return (
              <div key={type} className="mb-catalog__subsection">
                <div className="mb-catalog__header">{title}</div>
                <div className="mb-catalog__list">
                  {keys.length === 0 && (
                    <div className="mb-catalog__empty-hint">No {type.toLowerCase()} submodels yet</div>
                  )}
                  {keys.map((key) => {
                    ...
                    return (
                      <div
                        ...
                        draggable
                        onDragStart={(e) => onDragStartSubmodel(e, key, type)}
                        title={`Drag onto a ${type} AAS to add ${meta.label}`}
                      >
                        <div className="mb-catalog__tile-icon">
