...
export const AasShellNode = memo(function AasShellNode({ id, selected }: NodeProps) {
  ...
  const aasType = ns.aasType;
  const isConfigured = Boolean(identitySystemId.trim());

  const handleTypeChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setActiveAasNode(id);
    setAASType(e.target.value as AASType);
  }, [id, setActiveAasNode, setAASType]);
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div className="mb-shell-info-bar">
            ...
            <div className="mb-shell-info-actions">
              <select
                className={`mb-shell-type-select mb-shell-type-select--${aasType.toLowerCase()}`}
                value={aasType}
                onChange={handleTypeChange}
                onClick={(e) => e.stopPropagation()}
                onMouseDown={(e) => e.stopPropagation()}
                title="AAS type — selects Resource (ARSO) or Product (APSO) validation and which submodels apply"
              >
