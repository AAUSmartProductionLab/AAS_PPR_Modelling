...
import { makeHandleId } from '../connectionRules';
...
interface PropRow {
  ...
  /** Connection kind carried on the handle (see connectionRules). Omit for headers. */
  kind?: string;
  /** Element name carried on the handle (the thing a rule reads/writes). */
  item?: string;
...
function getRows(
  ...
  switch (key) {
    case 'Nameplate': {
      ...
      if (!np) return [];
      return ([
        np.ManufacturerName               && { id: 'np-mfr',  label: 'Manufacturer', value: mlpStr(np.ManufacturerName),               handleType: 'target', kind: 'nameplate', item: 'ManufacturerName' },
        np.SerialNumber                   && { id: 'np-sn',   label: 'Serial No.',   value: mlpStr(np.SerialNumber),                   handleType: 'target', kind: 'nameplate', item: 'SerialNumber' },
        np.ManufacturerProductDesignation && { id: 'np-prod', label: 'Product',      value: mlpStr(np.ManufacturerProductDesignation), handleType: 'target', kind: 'nameplate', item: 'Product' },
        np.YearOfConstruction             && { id: 'np-year', label: 'Year',         value: mlpStr(np.YearOfConstruction),             handleType: 'target', kind: 'nameplate', item: 'Year' },
        np.HardwareVersion                && { id: 'np-hw',   label: 'HW ver.',      value: mlpStr(np.HardwareVersion),                handleType: 'target', kind: 'nameplate', item: 'HardwareVersion' },
        np.SoftwareVersion                && { id: 'np-sw',   label: 'SW ver.',      value: mlpStr(np.SoftwareVersion),                handleType: 'target', kind: 'nameplate', item: 'SoftwareVersion' },
    ...
    case 'AID': {
      ...
      for (const [ifaceName, iface] of Object.entries(aid)) {
        ...
        for (const [k, p] of Object.entries(props)) {
          rows.push({ id: `aid-prop-${ifaceName}-${k}`, label: 'property', value: p?.key || k, handleType: 'target', kind: 'aid-property', item: p?.key || k, indent: true });
        }
        for (const [k, a] of Object.entries(actions)) {
          rows.push({ id: `aid-act-${ifaceName}-${k}`, label: 'action', value: a?.key || k, handleType: 'target', kind: 'aid-action', item: a?.key || k, indent: true });
        }
        for (const [k, e] of Object.entries(events)) {
          rows.push({ id: `aid-evt-${ifaceName}-${k}`, label: 'event', value: e?.key || k, handleType: 'target', kind: 'aid-event', item: e?.key || k, indent: true });
        }
      }
      return rows;
    }
    ...
    case 'Skills': {
      ...
      return Object.entries(skills).slice(0, 8).map(([name, skill]) => ({
        id: `sk-${name}`,
        label: name,
        value: skill?.interface ? `⇄ ${skill.interface}` : (skill?.callType ? `[${skill.callType}]` : '—'),
    ...
    case 'HierarchicalStructures': {
      ...
      return [
        { id: 'hs-entry',    label: 'EntryNode', value: hs.Name ?? identitySystemId, handleType: 'target', kind: 'bom-entry',    item: identitySystemId },
        { id: 'hs-haspart',  label: 'HasPart',   value: hasPart  > 0 ? `${hasPart} part${hasPart !== 1 ? 's' : ''}`             : '—', handleType: 'source', kind: 'bom-haspart',  item: 'HasPart' },
        { id: 'hs-ispartof', label: 'IsPartOf',  value: isPartOf > 0 ? `${isPartOf} parent${isPartOf !== 1 ? 's' : ''}`         : '—', handleType: 'source', kind: 'bom-ispartof', item: 'IsPartOf' },
        { id: 'hs-sameas',   label: 'SameAs',    value: sameAs   > 0 ? `${sameAs} equiv.`                                       : '—', handleType: 'both',   kind: 'bom-sameas',   item: 'SameAs' },
      ];
    }
              ...
              <div key={row.id} className={rowClass} style={{ position: 'relative' }}>
                ...
                {showTarget && (
                  <Handle
                    ...
                    id={makeHandleId('target', submodelKey, row.kind ?? 'ref', row.item ?? row.id)}
                ...
                {showSource && (
                  <Handle
                    ...
                    id={makeHandleId('source', submodelKey, row.kind ?? 'ref', row.item ?? row.id)}
